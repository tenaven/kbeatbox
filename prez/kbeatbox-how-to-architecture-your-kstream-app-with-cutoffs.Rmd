...
### Accessing the system time is a NO GO.
...
Testing with the system time is difficult :

You can not easily change the system time, because the whole machine
is dependent on the system time.
...
That works well for unit test but for acceptance testing
it is still an issue.

### Kafka is a great at replay!

What's great about Kafka is the ability to replay anything, production, acceptance, you name it.

For acceptance testing it is a fantastic ability, you can save the logs from previous days of production,
and compare the result in time as your software evolves.

Spotting the differences between version is easy if the system is :

 * deterministic (no race conditions for instance)
 * Not system time dependent
 * No threading effect
